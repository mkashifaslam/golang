...
func (cmd *Command) Print() {
	fmt.Printf("%v\n %v\n %s\n", cmd.App, cmd.SubCommand.Action, cmd.SubCommand.Args)
}

func New(app Cmd, action Act, args string) *Command {
	return &Command{App: app, SubCommand: SubCommand{Action: action, Args: args}}
}
...
func Run() {
	...
	Handler(cmd.App, cmd.SubCommand.Action, cmd.SubCommand.Args)
}
